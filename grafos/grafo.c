...
void DFS_Visit(GrafoA *G, int u, DFS *V, int *tempo)
{
    ...
    {
        if (V[pAux->id].cor == BRANCO)
        {
            V[pAux->id].pai = u;
            printf("Aresta arvore: ( %d - %d )\n", u, pAux->id);

            DFS_Visit(G, pAux->id, V, tempo);
        }
        else
        {
...
int main()
{
    ...
    adicionar_aresta_grafo_adj(0, 3, Ga);
    adicionar_aresta_grafo_adj(1, 2, Ga);
    adicionar_aresta_grafo_adj(2, 5, Ga);
    adicionar_aresta_grafo_adj(5, 4, Ga);
    adicionar_aresta_grafo_adj(5, 6, Ga);
    adicionar_aresta_grafo_adj(4, 1, Ga);
    adicionar_aresta_grafo_adj(6, 0, Ga);
    adicionar_aresta_grafo_adj(6, 3, Ga);
    adicionar_aresta_grafo_adj(6, 4, Ga);

    adicionar_aresta_grafo_adj(7, 8, Ga);

    adicionar_aresta_grafo_adj(9, 10, Ga);
    adicionar_aresta_grafo_adj(10, 11, Ga);
    adicionar_aresta_grafo_adj(11, 9, Ga);
